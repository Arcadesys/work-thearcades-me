...
export default function DraftingPanel({ lead, history, usage, claims }: Props) {
  ...
  return <>
    <section aria-labelledby="original-posting"><h2 id="original-posting">Original posting source</h2>
    ...
    <section aria-labelledby="fit-assessment"><h2 id="fit-assessment">Fit assessment and draft</h2>
      ...
      {reviewed.length === 0 ? <p role="status"><strong>Blocked:</strong> Mark at least one sourced claim as reviewed in the Resume truth section on the job desk.</p> : null}
    ...
    {history.assessments.map((item) => <section key={item.id} aria-label={`Assessment ${item.id}`}><h2>Assessment · {new Date(item.createdAt).toLocaleString()}</h2>
    ...
    {history.draft ? <section aria-labelledby="editable-draft"><h2 id="editable-draft">Editable application draft</h2>
      ...
      <h3>Truth snapshot used</h3><ul>{(history.draft.truthSnapshot ?? []).map((claim: Claim) => <li id={`claim-${encodeURIComponent(claim.id)}`} key={claim.id}><strong>{claim.claim}</strong> · {claim.sourceNote} · <Link href="/jobs#drafting-handoff">Claim {claim.id}</Link></li>)}</ul>
