...
import { listMcpTokens } from '@/lib/job-queue';
...
export default async function SettingsPage() {
  ...
  const [queries, status, tokens] = await Promise.all([listSearchQueries(), listSearchStatus(), listMcpTokens(ownerId)]);
  return <JobsShell active="settings"><header className="jobsPageHeader"><p className="jobsEyebrow">Discovery</p><h1>Search settings</h1><p>Connect and check your Google Alerts RSS feeds.</p></header>
    ...
    <section className="jobsPanel" style={{ marginTop: '1rem' }}><h2>LinkedIn email alerts</h2><p>Roles from your LinkedIn job alert emails can be imported as unverified leads. These imports run when requested; the daily feed scan above checks Google Alerts only.</p><p><Link href="/jobs/leads">Review imported LinkedIn leads →</Link></p></section>
    <McpTokenManager tokens={tokens as any} />
  </JobsShell>;
}
