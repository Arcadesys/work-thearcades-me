...
export default function PrivacyPage() {
  return <div className="services-page">
    ...
    <main id="main" tabIndex={-1} className="privacy-copy">
      ...
      <p>Analytics records the public page path, site hostname, referring website domain, and validated campaign labels. It excludes email addresses, form contents, URL fragments, and other query parameters. When you request Work build notes, the address is checked against Kit’s exact-address status lookup to suppress blocked contacts, then sent to Postmark for a one-time verification message. No Kit subscriber, form, or tag is written until you explicitly verify the link. Kit may then send its own confirmation before the notes begin. Redis temporarily stores an encrypted address and keyed digests for one-time link handling and abuse limits; raw IP addresses are not stored there. Newsletter and external booking services handle information separately when you choose to use them.</p>
