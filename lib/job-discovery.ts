...
export async function listSearchQueries(sql: Sql = db()) {
  ...
  return await sql`SELECT id,lane,location,query,enabled FROM job_search_queries ORDER BY id`;
}

export async function saveSearchQueries(queries: Array<{ id: string; query: string; enabled: boolean }>, sql: Sql = db()): Promise<void> {
  ...
  const payload = JSON.stringify(queries.map((item) => ({ ...item, query: item.query.trim() })));
  await sql`WITH changes AS (
    SELECT * FROM jsonb_to_recordset(${payload}::jsonb) AS x(id text, query text, enabled boolean)
  ) UPDATE job_search_queries q SET query=c.query,enabled=c.enabled,updated_at=now()
...
export async function ingestSearchResults(results: SearchResult[], queryId: string, sql: Sql = db()): Promise<{ seen: number; created: number }> {
  ...
  for (const result of results) {
    ...
    leads.push({ id: leadIdForUrl(sourceUrl), source_url: sourceUrl, title: result.title.slice(0,250), organization: result.profile?.long_name ?? result.profile?.name ?? '', snippet: result.description?.slice(0,3000) ?? '', source: `brave:${queryId}` });
  }
...
export type ScanDependencies = { sql?: Sql; fetcher?: typeof fetch; now?: Date; apiKey?: string };
export async function runDailySearch({ sql = db(), fetcher = fetch, now = new Date(), apiKey = process.env.BRAVE_SEARCH_API_KEY }: ScanDependencies = {}) {
  ...
  try {
    if (!apiKey) throw new Error('BRAVE_SEARCH_API_KEY is not configured. No search calls were made.');
    await initializeSearchQueries(sql);
    const queries = await sql`SELECT id,query FROM job_search_queries WHERE enabled=true ORDER BY id` as Array<{ id: string; query: string }>;
    for (const item of queries) {
      if (!await reserveCall(sql)) throw new Error(`Monthly search call cap (${SEARCH_CAP}) reached.`);
      calls++;
      attempted++;
      const url = new URL('https://api.search.brave.com/res/v1/web/search');
      url.searchParams.set('q', item.query);
      url.searchParams.set('count', '10');
      const response = await fetcher(url, { headers: { 'X-Subscription-Token': apiKey, Accept: 'application/json' }, signal: AbortSignal.timeout(8000), cache: 'no-store' });
      if (!response.ok) throw new Error(`Brave search returned HTTP ${response.status} for ${item.id}.`);
    ...
    await sql`UPDATE job_search_runs SET status='succeeded',completed_at=now(),queries_attempted=${attempted},results_seen=${seen},new_leads=${added},calls_used=${calls} WHERE run_date=${date}`;
    return { skipped: false, status: 'succeeded', attempted, seen, added, calls };
  } catch (error) {
