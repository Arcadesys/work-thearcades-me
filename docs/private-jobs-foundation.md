...
Apply `db/migrations/0002_job_discovery.sql` to enable the private lead inbox and application pipeline. The daily `/api/jobs/discovery` cron requires `CRON_SECRET` as a Bearer token and uses `BRAVE_SEARCH_API_KEY` for Brave Web Search. Missing Brave configuration is recorded as a failed run without making a search call. Calls are reserved atomically up to 300 per calendar month. Each UTC date can start one run; duplicate deliveries return a skipped result. Add discovered postings as unverified leads: snippets are only discovery hints, and a person must open the original URL to verify status. Manual leads and saved pipeline decisions remain private to the authorized GitHub account.
