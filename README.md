...
- **Form and endpoint:** replaced the ActiveCampaign embed with an accessible first-party form and `/api/kit/subscribe`. Configure server-only `KIT_API_KEY` and numeric `KIT_FORM_ID`; the route only adds a subscriber to the configured Kit form when Kit's upsert response says the subscriber is `inactive`. If Kit reports `active`, the route makes no form-add call and returns a recoverable message, since global active status does not prove confirmation of this form. It sends no referrer or page URL to Kit.
- **Confirmation behavior:** success copy asks readers to check their inbox and says the subscription starts after confirmation. The daily Vercel cron route `/api/kit/reconcile-confirmations` is disabled unless `KIT_RECONCILE_ENABLED=true`. Keep this variable unset: Kit reports global subscriber activity, and active membership on a form has not been established as proof that this specific form's double opt-in was confirmed. Enabling the cron could tag someone before Work-form confirmation. Do not enable it until a reliable per-form confirmation signal is verified. There is no Kit automation or email-sending code in this integration.
- **Reconciliation configuration:** the disabled code path reads active members of the configured form and applies the `Interest: Work / AI` tag to active form members missing the tag. It requires server-only `KIT_API_KEY`, `KIT_FORM_ID`, `KIT_WORK_TAG_ID`, a random `CRON_SECRET` (at least 16 characters), and explicit `KIT_RECONCILE_ENABLED=true`. Kit's Free plan includes API access; Vercel Hobby cron runs once per day. The endpoint reads all cursor pages before writing tags, and is idempotent across retries. References: [Kit's active form-member API](https://developers.kit.com/api-reference/forms/list-subscribers-for-a-form), [tag-member API](https://developers.kit.com/api-reference/tags/list-subscribers-for-a-tag), [tag subscriber API](https://developers.kit.com/api-reference/tags/tag-a-subscriber), [Kit Free plan](https://help.kit.com/en/articles/16627071-the-kit-free-plan), [Vercel cron auth](https://vercel.com/docs/cron-jobs/manage-cron-jobs).
